...
The viewer's animation-shadow mode disables the character's standard cast/receive shadows and uses `ContactShadow` from `lab/contact-shadow.ts` for its ground projection. It captures the actual caster geometry along the light direction, including skeletal and morph animation, then tints and softens that mask on the horizontal stage. Disconnected objects retain separate footprints; overlapping footprints merge without stacking dark ovals. Elevated objects cast displaced shadows along the same light direction. This is an app-owned helper with `update`, `reset` and `dispose`, separate from `InkPass`. Capture once per rendered pose per WebGL context, before showing the ground receiver. It supports the viewer's flat rectangular/circular stages; uneven terrain needs a different receiver. The library pass does not change lights or `castShadow` settings.

Run `/tests/contact-shadow.html` under Vite for independent casters, movement, morphs, skeletal poses, light/height projection, multiple render contexts and failure-state restoration.
