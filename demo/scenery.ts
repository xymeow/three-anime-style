...
export function withPaintedStage(subject: T.Object3D) {
  ...
  mesh(
    root,
    new T.CylinderGeometry(2.7, 2.75, 0.15, 64),
    material("#b8c0a2"),
    0,
    -0.08,
    0,
  ...
  for (const [x, height, color] of [
    ...
    const mound = mesh(
      root,
      new T.SphereGeometry(1, 32, 24, 0, Math.PI * 2, 0, Math.PI / 2),
      material(color),
      x,
      0,
      -1.45,
    );
    mound.scale.set(1.15, height, 0.7);
  }
  return root;
}
export function isScenery(mesh: T.Mesh) {
  return mesh.userData.inkPaint === true;
}
