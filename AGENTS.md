...
- Source map: `src/material.ts` adapts materials; `src/pass.ts` owns outlines/post effects; `src/brush.ts` creates brush textures; `src/clock.ts` samples animation time. `demo/main.ts` is the single comparison viewer; `demo/catalog.ts` owns every example and imported-model loader. `lab/` provides scenery and contact-shadow helpers; `lab.html` only redirects old links.
